...
def verify_checksums(sums_path: Path) -> None:
    ...
    if len(lines) != 7:
        raise SystemExit(
            ...
            "portable zip, product report, VPS compatibility report, update manifest, and signing readiness report."
        )
    for line in lines:
        try:
...
def main() -> None:
    ...
    release_asset_paths = [*core_asset_paths, update_manifest_path, signing_report_path]
