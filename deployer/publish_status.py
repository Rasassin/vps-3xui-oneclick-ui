...
def run_command(args: list[str], timeout: int = 12) -> tuple[int, str]:
    ...
    return result.returncode, sanitize_command_output((result.stdout + "\n" + result.stderr).strip())
...
def collect_publish_checks(version: str = APP_VERSION) -> list[PublishCheck]:
    return [
        check_remote_config(),
        check_branch_sync(),
        check_worktree(),
...
def publish_overall_status(checks: list[PublishCheck]) -> str:
...
def publish_report_text(checks: list[PublishCheck], version: str = APP_VERSION) -> str:
...
Overall status: `{publish_overall_status(checks)}`
...
def write_publish_report(checks: list[PublishCheck] | None = None, version: str = APP_VERSION) -> Path:
    dist_dir = PROJECT_ROOT / "dist"
    dist_dir.mkdir(parents=True, exist_ok=True)
    path = dist_dir / f"PUBLISH_READINESS_v{version}.md"
    path.write_text(publish_report_text(checks or collect_publish_checks(version), version), encoding="utf-8")
    return path
